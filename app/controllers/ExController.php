			...
			$PendingOrders = Orders::find('all',
				...
				foreach ($PendingOrders as $PO){
					...
					if((float)$PO['Amount']>(float)($Amount)){
						...
						$data = array(
							'Commission.Amount' => (float)round($PrevCommAmount,8),
							'Amount' => (float)($Amount),
							'Completed' => 'Y',
							'Transact.id'=> $order_id,
							'Transact.username' => $user['username'],
...
/////////////////////////////////////////////////////////////////////////////////////////////							
					...
					if((float)$PO['Amount']<(float)($Amount)){
						...
						$data = array(
							'Commission.Amount' => (float)round($CurrCommAmount,8),
							'Amount' => (float)($PO['Amount']),
							'Completed' => 'Y',
							'Transact.id'=> $PO['_id'],
							'Transact.username' => $PO['username'],
...
///////////////////////////////////////////////////////////////////////////////////////////////////////							
		...
		return compact('title','details','SellOrders','BuyOrders','TotalSellOrders','TotalBuyOrders','YourOrders','YourCompleteOrders','keywords','description','settings');
	}
