...
<h4>Hi <?=$compact['data']['username']?>,</h4>
<p>You have requested to withdraw <strong><?=abs($compact['data']['Amount'])?> LTC</strong> from <?=COMPANY_URL?>.</p>
<p>Click on the link below to confirm the transfer. </p>
<p>If you did not authorize this withdrawal to the litecoin address: <strong><?=$compact['data']['address']?></strong> please <strong style="color:#FF0000">do not</strong> click on the link.</p>
<a href="https://<?=COMPANY_URL?>/users/paymentltcconfirm/<?=$compact['data']['verify.payment']?>">https://<?=COMPANY_URL?>/users/paymentltcconfirm/<?=$compact['data']['verify.payment']?></a>
